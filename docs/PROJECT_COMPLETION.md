...
with a fully described AKS secondary environment ready for deployment.
The GitLab-CI pipeline defines unit + integration + E2E + performance +
ZAP + Trivy stages. The live GKE environment is observed by
kube-prometheus-stack, Loki, Jaeger and Kiali, uses Istio sidecars and
development-safe resilience policies, and includes Chaos Mesh for
...
significantly, the Azure deployment and multi-cloud DR failover have
been *designed and scripted* but not yet applied or rehearsed
end-to-end.
...
2. **Azure deployment and DR drill execution** — the Terraform modules
   and runbook in
   [`OPERATIONS.md`](OPERATIONS.md) §6 is complete and scripted, but the
   AKS environment has not been applied and the first cross-cloud
   failover drill has not yet been performed end-to-end. *Next:* apply
   the stage environment under an enabled Azure subscription, run the
   drill, and record measured RPO/RTO in `docs/dr-drills/`.
